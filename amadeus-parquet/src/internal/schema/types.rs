...
impl<'a> PrimitiveTypeBuilder<'a> {
	...
	/// Sets [`Repetition`](crate::internal::basic::Repetition) for this field and returns itself.
	pub fn with_repetition(mut self, repetition: Repetition) -> Self {
		self.repetition = repetition;
		self
	}

	/// Sets [`LogicalType`](crate::internal::basic::LogicalType) for this field and returns itself.
	...
	pub fn build(self) -> Result<Type> {
		...
		if self.physical_type == PhysicalType::FixedLenByteArray && self.length < 0 {
		...
		match self.logical_type {
			...
			LogicalType::Interval => {
				if self.physical_type != PhysicalType::FixedLenByteArray || self.length != 12 {
					return Err(general_err!(
...
impl BasicTypeInfo {
	...
	/// Returns [`Repetition`](crate::internal::basic::Repetition) value for the type.
	pub fn repetition(&self) -> Repetition {
		assert!(self.repetition.is_some());
		self.repetition.unwrap()
	}

	/// Sets [`Repetition`](`crate::internal::basic::Repetition`) value for the type.
	pub fn set_repetition(&mut self, repetition: Option<Repetition>) {
		self.repetition = repetition;
	}

	/// Returns [`LogicalType`](crate::internal::basic::LogicalType) value for the type.
...
impl ColumnDescriptor {
	...
	pub fn new(
		primitive_type: TypePtr, root_type: Option<TypePtr>, max_def_level: i16,
		max_rep_level: i16, path: ColumnPath,
	) -> Self {
		Self {
			primitive_type,
			root_type,
	...
	/// Returns self type [`Type`](crate::internal::schema::types::Type) for this leaf column.
	pub fn self_type(&self) -> &Type {
		self.primitive_type.as_ref()
	}

	/// Returns self type [`TypePtr`](crate::internal::schema::types::TypePtr)  for this leaf
	/// column.
	pub fn self_type_ptr(&self) -> TypePtr {
		self.primitive_type.clone()
	}

	/// Returns root [`Type`](crate::internal::schema::types::Type) (most top-level parent field)
	...
	/// Returns [`LogicalType`](crate::internal::basic::LogicalType) for this column.
	pub fn logical_type(&self) -> LogicalType {
...
impl SchemaDescriptor {
	...
	/// Returns column root [`Type`](crate::internal::schema::types::Type) for a field position.
	pub fn get_column_root(&self, i: usize) -> &Type {
		let result = self.column_root_of(i);
		result.as_ref()
	}

	/// Returns column root [`Type`](crate::internal::schema::types::Type) pointer for a field
	/// position.
	...
	/// Returns schema as [`Type`](crate::internal::schema::types::Type).
...
fn build_tree(
	tp: TypePtr, root_tp: TypePtr, base_tp: TypePtr, mut max_rep_level: i16,
	mut max_def_level: i16, leaves: &mut Vec<ColumnDescPtr>,
	leaf_to_base: &mut HashMap<usize, TypePtr>, path_so_far: &mut Vec<String>,
) {
...
mod tests {
	...
	fn test_primitive_type() {
		...
		if let Ok(tp) = result {
			...
			assert_eq!(basic_info.logical_type(), LogicalType::Int32);
		...
		result = Type::primitive_type_builder("foo", PhysicalType::Int64)
			.with_repetition(Repetition::Required)
			.with_logical_type(LogicalType::Uint8)
			.build();
		assert!(result.is_err());
		if let Err(e) = result {
		...
		result = Type::primitive_type_builder("foo", PhysicalType::Int32)
			.with_repetition(Repetition::Required)
			.with_logical_type(LogicalType::TimeMicros)
			.build();
		assert!(result.is_err());
		if let Err(e) = result {
	...
	fn test_column_descriptor_helper() -> Result<()> {
		let tp = Type::primitive_type_builder("name", PhysicalType::ByteArray)
			.with_logical_type(LogicalType::Utf8)
			.build()?;
		...
		assert_eq!(descr.physical_type(), PhysicalType::ByteArray);
	...
	fn test_schema_descriptor_helper() -> Result<()> {
		...
		let intc = Type::primitive_type_builder("c", PhysicalType::ByteArray)
			.with_repetition(Repetition::Repeated)
			.with_logical_type(LogicalType::Utf8)
			.build()?;
	...
	fn test_get_physical_type_primitive() {
		...
		let f = Type::primitive_type_builder("f", PhysicalType::ByteArray)
			.build()
			.unwrap();
		assert_eq!(f.get_physical_type(), PhysicalType::ByteArray);
	}
	...
	fn test_check_contains_primitive_primitive() {
		...
		let f1 = Type::primitive_type_builder("f", PhysicalType::Int32)
			...
			.unwrap();
		let f2 = Type::primitive_type_builder("f", PhysicalType::Int32)
			.with_logical_type(LogicalType::Uint16)
			.build()
			.unwrap();
		assert!(f1.check_contains(&f2));
