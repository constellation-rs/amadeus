...
use crate::internal::{
	basic::*, data_type::*, encodings::rle::RleEncoder, errors::{ParquetError, Result}, schema::types::ColumnDescPtr, util::{
		bit_util::{self, log2, num_required_bits, BitWriter}, hash_util, memory::{Buffer, ByteBuffer, ByteBufferPtr, MemTrackerPtr}
	}
};
...
pub trait Encoder<T: DataType> {
	...
	fn put(&mut self, values: &[T::Type]) -> Result<()>;
	...
	fn put_spaced(&mut self, values: &[T::Type], valid_bits: &[u8]) -> Result<usize> {
...
impl<T: DataType> Encoder<T> for PlainEncoder<T> {
	default fn put(&mut self, values: &[T::Type]) -> Result<()> {
		let bytes = unsafe {
			slice::from_raw_parts(
				values as *const [T::Type] as *const u8,
				mem::size_of::<T::Type>() * values.len(),
			)
		};
...
impl<T: DataType> DictEncoder<T> {
	...
	#[inline]
	fn put_one(&mut self, value: &T::Type) -> Result<()> {
	...
	#[inline]
	fn add_dict_key(&mut self, value: T::Type) {
...
trait DictEncodedSize<T: DataType> {
	fn get_encoded_size(&self, value: &T::Type) -> usize;
}

impl<T: DataType> DictEncodedSize<T> for DictEncoder<T> {
	#[inline]
	default fn get_encoded_size(&self, _: &T::Type) -> usize {
		mem::size_of::<T::Type>()
	}
}
...
impl<T: DataType> Encoder<T> for RleValueEncoder<T> {
	#[inline]
	default fn put(&mut self, _values: &[T::Type]) -> Result<()> {
...
impl<T: DataType> DeltaBitPackEncoder<T> {
	...
	fn write_page_header(&mut self) {
		...
		self.page_header_writer
			.put_vlq_int(self.block_size as u64)
			.unwrap();
...
mod tests {
	...
	use super::*;
	use crate::internal::{
		decoding::{get_decoder, Decoder, DictDecoder, PlainDecoder}, schema::types::{ColumnDescPtr, ColumnDescriptor, ColumnPath, Type as SchemaType}, util::{
			memory::MemTracker, test_common::{
				benches::{col_desc, gen_10, gen_100, gen_1000, gen_test_strs}, random_bytes, RandGen
			}
		}
	};
	...
	fn test_estimated_data_encoded_size() {
		fn run_test<T: DataType>(
			...
			max_size: usize, flush_size: usize,
		) {
		...
		// ::DeltaBinaryPacked
		run_test::<Int32Type>(Encoding::DeltaBinaryPacked, -1, &vec![123; 1024], 0, 35, 0);
	...
	impl<T: DataType> EncodingTester<T> for T {
		fn test_internal(enc: Encoding, total: usize, type_length: i32) -> Result<()> {
			...
			let mut result_data = vec![T::Type::default(); total];
			...
			for i in 0..total {
				if bit_util::get_bit(&valid_bits[..], i) {
					...
					assert_eq!(result_data[i], T::Type::default());
				}
			}
	...
	fn put_and_get<T: DataType>(
		encoder: &mut Box<dyn Encoder<T>>, decoder: &mut Box<dyn Decoder<T>>, input: &[T::Type],
		output: &mut [T::Type],
	) -> Result<usize> {
		encoder.put(input)?;
		let data = encoder.flush_buffer()?;
	...
	fn bench_encoding<T: DataType>(
		bench: &mut Bencher, bytes: usize, values: Vec<T::Type>, mut encoder: Box<dyn Encoder<T>>,
	) {
		bench.bytes = bytes as u64;
		bench.iter(|| {
			encoder.put(&values[..]).expect("put() should be OK");
			let _ = encoder.flush_buffer().expect("flush_buffer() should be OK");
		})
	}
