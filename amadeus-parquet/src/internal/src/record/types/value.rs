...
use std::{
	collections::HashMap, convert::TryInto, hash::{Hash, Hasher}
};

use crate::{
	basic::{LogicalType, Repetition, Type as PhysicalType}, column::reader::ColumnReader, data_type::Decimal, errors::{ParquetError, Result}, record::{
		reader::ValueReader, schemas::{
			BoolSchema, BsonSchema, ByteArraySchema, DateSchema, DecimalSchema, EnumSchema, F32Schema, F64Schema, GroupSchema, I16Schema, I32Schema, I64Schema, I8Schema, JsonSchema, ListSchema, ListSchemaType, OptionSchema, StringSchema, TimeSchema, TimestampSchema, U16Schema, U32Schema, U64Schema, U8Schema, ValueSchema
		}, types::{
			list::parse_list, map::parse_map, Bson, Date, Downcast, Enum, Group, Json, List, Map, Time, Timestamp, ValueRequired
		}, Record
...
impl Record for Value {
		...
		if repetition.is_some() && schema.is_primitive() {
			value = Some(
				match (
					...
					(PhysicalType::Int96, _) => ValueSchema::Timestamp(TimestampSchema::Int96),
	...
	fn reader(
		...
		match *schema {
			...
			ValueSchema::Decimal(ref schema) => ValueReader::Decimal(<Decimal as Record>::reader(
				schema, path, def_level, rep_level, paths, batch_size,
			)),
			ValueSchema::ByteArray(ref schema) => ValueReader::ByteArray(
				<Vec<u8> as Record>::reader(schema, path, def_level, rep_level, paths, batch_size),
			),
			ValueSchema::Bson(ref schema) => ValueReader::Bson(<Bson as Record>::reader(
				schema, path, def_level, rep_level, paths, batch_size,
			)),
			ValueSchema::String(ref schema) => ValueReader::String(<String as Record>::reader(
				schema, path, def_level, rep_level, paths, batch_size,
			)),
			...
			ValueSchema::Group(ref schema) => ValueReader::Group(<Group as Record>::reader(
				schema, path, def_level, rep_level, paths, batch_size,
			)),
