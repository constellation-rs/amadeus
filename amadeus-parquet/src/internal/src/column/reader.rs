...
use std::{
	cmp::{max, min}, collections::HashMap, mem
};

use super::page::{Page, PageReader};
use crate::{
	basic::*, data_type::*, encodings::{
		decoding::{get_decoder, Decoder, DictDecoder, PlainDecoder}, levels::LevelDecoder
	}, errors::{ParquetError, Result}, schema::types::ColumnDescPtr, util::memory::ByteBufferPtr
};
...
pub fn get_column_reader(
	col_descr: ColumnDescPtr, col_page_reader: Box<dyn PageReader>,
) -> ColumnReader {
...
impl<T: DataType> ColumnReaderImpl<T> {
	...
	#[inline]
	pub fn read_batch(
		&mut self, batch_size: usize, mut def_levels: Option<&mut [i16]>,
		mut rep_levels: Option<&mut [i16]>, values: &mut [T::Type],
	) -> Result<(usize, usize)> {
	...
	fn set_current_page_encoding(
		&mut self, mut encoding: Encoding, buffer_ptr: &ByteBufferPtr, offset: usize, len: usize,
	) -> Result<()> {
...
mod tests {
	...
	use crate::{
		basic::Type as PhysicalType, column::page::Page, encodings::encoding::{DictEncoder, Encoder}, schema::types::{ColumnDescriptor, ColumnPath, Type as SchemaType}, util::{
			memory::MemTracker, test_common::{
				page_util::{DataPageBuilder, DataPageBuilderImpl}, random_numbers_range
			}
		}
	};
	...
	fn test_read_batch_int32(
		batch_size: usize, values: &mut [i32], def_levels: Option<&mut [i16]>,
		...
		T::Type: PartialOrd + SampleUniform + Copy,
	{
		...
		fn plain_v1(
			&mut self, desc: ColumnDescPtr, num_pages: usize, num_levels: usize, batch_size: usize,
			min: T::Type, max: T::Type,
		) {
		...
		fn test_read_batch_general(
			&mut self, desc: ColumnDescPtr, encoding: Encoding, num_pages: usize,
			num_levels: usize, batch_size: usize, min: T::Type, max: T::Type, use_v2: bool,
		) {
		...
		fn test_read_batch(
			&mut self, desc: ColumnDescPtr, encoding: Encoding, num_pages: usize,
			num_levels: usize, batch_size: usize, min: T::Type, max: T::Type,
			values: &mut [T::Type], mut def_levels: Option<&mut [i16]>,
			mut rep_levels: Option<&mut [i16]>, use_v2: bool,
		) {
	...
	fn make_pages<T: DataType>(
		desc: ColumnDescPtr, encoding: Encoding, num_pages: usize, levels_per_page: usize,
		min: T::Type, max: T::Type, def_levels: &mut Vec<i16>, rep_levels: &mut Vec<i16>,
		values: &mut Vec<T::Type>, pages: &mut VecDeque<Page>, use_v2: bool,
	) where
		T::Type: PartialOrd + SampleUniform + Copy,
	{
