...
use std::{
	fs::File, io::{Seek, SeekFrom, Write}, rc::Rc
};
...
pub trait FileWriter {
	...
	fn close_row_group(&mut self, row_group_writer: Box<dyn RowGroupWriter>) -> Result<()>;
...
impl SerializedFileWriter {
	...
	fn finalise_row_group_writer(
		&mut self, mut row_group_writer: Box<dyn RowGroupWriter>,
	) -> Result<()> {
