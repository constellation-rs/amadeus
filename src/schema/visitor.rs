...
use crate::{
	basic::{LogicalType, Repetition}, errors::{ParquetError::General, Result}, schema::types::{Type, TypePtr}
};
...
pub trait TypeVisitor<R, C> {
				...
				match list_item.as_ref() {
					Type::PrimitiveType { .. } => {
						if list_item.get_basic_info().repetition() == Repetition::Repeated {
							self.visit_list_with_item(list_type.clone(), list_item, context)
						} else {
							Err(General(
	...
	fn dispatch(&mut self, cur_type: TypePtr, context: C) -> Result<R> {
		if cur_type.is_primitive() {
			...
			match cur_type.get_basic_info().logical_type() {
				...
				LogicalType::Map | LogicalType::MapKeyValue => self.visit_map(cur_type, context),
	...
	fn visit_list_with_item(
		&mut self, list_type: TypePtr, item_type: &Type, context: C,
	) -> Result<R>;
}
...
mod tests {
	...
	use crate::{
		basic::Type as PhysicalType, errors::Result, schema::{
			parser::parse_message_type, types::{Type, TypePtr}
		}
	};
	use std::rc::Rc;
	...
	impl TypeVisitor<bool, TestVisitorContext> for TestVisitor {
		...
		fn visit_struct(
			&mut self, struct_type: TypePtr, _context: TestVisitorContext,
		) -> Result<bool> {
			assert_eq!(
		...
		fn visit_list_with_item(
			&mut self, list_type: TypePtr, item_type: &Type, _context: TestVisitorContext,
		) -> Result<bool> {
			assert_eq!(
