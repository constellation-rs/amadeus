...
use std::{
	collections::HashMap, convert::{TryFrom, TryInto}, fmt::{self, Display}, result
};
use sum::{Sum2, Sum3};

use crate::{
	basic::Repetition, column::reader::ColumnReader, data_type::{Int96, Int96Type}, errors::{ParquetError, Result}, record::{
		reader::{I96Reader, MapReader}, schemas::{DateSchema, I32Schema, I64Schema, TimeSchema, TimestampSchema}, triplet::TypedTripletIter, types::{downcast, Value}, Reader, Record
			...
			TimeSchema::Micros => Sum2::A(MapReader(
				...
				|micros: i64| {
					micros
						...
						.ok_or_else(|| {
							ParquetError::General(format!("Invalid Time Micros {}", micros))
						})
				},
			)),
			TimeSchema::Millis => Sum2::B(MapReader(
				...
				|millis: i32| {
					millis
						...
						.ok_or_else(|| {
							ParquetError::General(format!("Invalid Time Millis {}", millis))
						})
				},
			)),
		}
