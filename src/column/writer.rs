...
use crate::{
	basic::{Compression, Encoding, PageType, Type}, column::page::{CompressedPage, Page, PageWriteSpec, PageWriter}, compression::{create_codec, Codec}, data_type::*, encodings::{
		encoding::{get_encoder, DictEncoder, Encoder}, levels::{max_buffer_size, LevelEncoder}
	}, errors::{ParquetError, Result}, file::{
		metadata::ColumnChunkMetaData, properties::{WriterProperties, WriterPropertiesPtr, WriterVersion}
	}, schema::types::ColumnDescPtr, util::memory::{ByteBufferPtr, MemTracker}
};
...
pub fn get_column_writer(
	descr: ColumnDescPtr, props: WriterPropertiesPtr, page_writer: Box<dyn PageWriter>,
) -> ColumnWriter {
...
impl<T: DataType> ColumnWriterImpl<T> {
	...
	pub fn write_batch(
		&mut self, values: &[T::Type], def_levels: Option<&[i16]>, rep_levels: Option<&[i16]>,
	) -> Result<usize> {
	...
	fn should_add_data_page(&self) -> bool {
		match self.dict_encoder {
			...
			None => self.encoder.estimated_data_encoded_size() >= self.props.data_pagesize_limit(),
		}
	}
	...
	#[inline]
	fn encode_levels_v1(
		&self, encoding: Encoding, levels: &[i16], max_level: i16,
	) -> Result<Vec<u8>> {
...
mod tests {
	...
	use crate::{
		column::{
			page::PageReader, reader::{get_column_reader, get_typed_column_reader, ColumnReaderImpl}
		}, file::{
	...
	fn column_roundtrip_random<'a, T: DataType>(
		file_name: &'a str, props: WriterProperties, max_size: usize, min_value: T::Type,
		max_value: T::Type, max_def_level: i16, max_rep_level: i16,
	) where
	...
	fn column_roundtrip<'a, T: DataType>(
		file_name: &'a str, props: WriterProperties, values: &[T::Type],
		def_levels: Option<&[i16]>, rep_levels: Option<&[i16]>,
	) {
	...
	fn column_write_and_get_metadata<T: DataType>(
		props: WriterProperties, values: &[T::Type],
	) -> ColumnChunkMetaData {
		let page_writer = get_test_page_writer();
	...
	fn check_encoding_write_support<T: DataType>(
		version: WriterVersion, dict_enabled: bool, data: &[T::Type],
		dictionary_page_offset: Option<i64>, encodings: &[Encoding],
	) {
		let props = WriterProperties::builder()
	...
	fn read_fully<T: DataType>(
		mut reader: ColumnReaderImpl<T>, batch_size: usize, mut def_levels: Option<&mut Vec<i16>>,
		mut rep_levels: Option<&mut Vec<i16>>, values: &mut [T::Type],
	) -> (usize, usize) {
	...
	fn get_test_column_writer<T: DataType>(
		page_writer: Box<dyn PageWriter>, max_def_level: i16, max_rep_level: i16,
		props: WriterPropertiesPtr,
	) -> ColumnWriterImpl<T> {
	...
	fn get_test_column_reader<T: DataType>(
		page_reader: Box<dyn PageReader>, max_def_level: i16, max_rep_level: i16,
	) -> ColumnReaderImpl<T> {
		let descr = Rc::new(get_test_column_descr::<T>(max_def_level, max_rep_level));
	...
	fn get_test_column_descr<T: DataType>(
		max_def_level: i16, max_rep_level: i16,
	) -> ColumnDescriptor {
